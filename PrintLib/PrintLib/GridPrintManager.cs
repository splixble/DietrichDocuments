...
namespace PrintLib
{
    ...
    {
        public SolidBrush NullCellBrush { get { return _NullCellBrush; } }
        SolidBrush _NullCellBrush = new SolidBrush(Color.DarkGray);
        SolidBrush _ErrorBrush = new SolidBrush(Color.Red);
        SolidBrush _BlackBrush = new SolidBrush(Color.Black);
        SolidBrush _DisabledTextBrush = new SolidBrush(DNColor.NullCells);
        ...
        public GridPrintManager(DataGridView grid)
        {
            m_Grid = grid;
         ...
         */
        GridPrintBookmark SetUpCoords(DNPrintDocument printDoc, Graphics graphics, Point parentOffset)
        {
            // Initialize things:
            ...
            DataGridViewColumn[] columnsInDisplayOrder = new DataGridViewColumn[Grid.Columns.Count];
            ...
            int lowestGridPos = printDoc.MarginBounds.Bottom - DNPrintDocument.m_FooterHeight;
        ...
        public bool PrintGrid(DNPrintDocument printDoc, Graphics graphics, Point parentOffset, bool printPage, ref PrintBookmark bookmark, out bool printedAnything)
        {
        ...
        void PrintGridPage(DNPrintDocument printDoc, Graphics graphics, GridPrintBookmark gridBookmark)
        {
            for (int gridIndex = 0; gridIndex < gridBookmark.CurrentPageCoords.Grids.Count; gridIndex++)
            {
                ...
                if (onFirstPage && firstGridOnPage)
                {
                    ...
                    graphics.DrawString("Grid 1", // Grid.GridPrintHeading,
                ...
                // REMOVED Dictionary<GridBanner, object> bannersPrinted = new Dictionary<GridBanner, object>();
                ...
                for (int nc = 0; nc < gridCoords.ColumnCoords.Count; nc++)
                {
                    ...
                    SolidBrush br = BrushCache.GetBrush(Color.Tan); // PLACEHOLD ...Color.LighterSage); 
                    graphics.FillRectangle(br, columnHeaderBounds);


                    // Paint column header image, if applicable:
                    // REMOVED Grid.PaintColumnHeaderImage(columnHeaderBounds,
                    //    nColNum, graphics, true);

                    // Write column heading:
                    PaintColumnHeaderText(Grid, columnHeaderBounds,
        ...
        public void PaintColumnHeaderText(DataGridView grid, Rectangle cellBounds, int columnIndex,
            Graphics graphicsOb, decimal scaleFactor, bool forPrint)
        {
            DataGridViewColumnHeaderCell headerCell = grid.Columns[columnIndex].HeaderCell;
            /* REMOVED
            GridColumnTag tag = GetColumnTag(columnIndex); // may be null

            ConvertibleGraphicsContext cGraphics =
                new ConvertibleGraphicsContext(graphicsOb, forPrint);
            */

            Rectangle textBounds = new Rectangle(cellBounds.X, cellBounds.Y,
            ...
            /* REMOVED
            if (tag != null && tag.HasFlag(GridColumnTag.TagFlags.VerticallyRotatedHeaderText))
            {
                textBounds.Width -= 2; // keep a little space to the right of the vertical text
            ...
            */
            {
                CellAlignmentToStringFormatAlignment(headerCell.Style.Alignment, stringFormat);
            }
            graphicsOb.DrawString(grid.Columns[columnIndex].HeaderText,
                Utils.ScaleFont(grid.ColumnHeadersDefaultCellStyle.Font, scaleFactor),
                (grid.Enabled ? _BlackBrush : _DisabledTextBrush), textBounds, stringFormat);
        }

        public static void CellAlignmentToStringFormatAlignment(DataGridViewContentAlignment cellAlignment,
            StringFormat strFormat)
        {
            // To handle any text alignment of cells, check cell.InheritedStyle.Alignment:
        ...
        */
        ...
        public virtual int MeasureCellHeight(DNPrintDocument printDoc,
            int nRowIndex, int nColumnIndex, Graphics graphicsOb)
        {
            ...
            if ( // REMOVED !Grid.Banners.ContainsCell(nRowIndex, nColumnIndex) &&
                ...
                cell.InheritedStyle.WrapMode == DataGridViewTriState.True)
            {
                ...
                CellAlignmentToStringFormatAlignment(cell.InheritedStyle.Alignment, strFormat);
        ...
        protected virtual void PrintCell(DNPrintDocument printDoc,
            int nRowIndex, int nColumnIndex, Rectangle cellBounds, Graphics graphicsOb)
        {
            ...
            */
            ...
            GetCellStyle(m_Grid, nRowIndex, nColumnIndex, out cellStyle);
            ...
            try
            {
                // REMOVED if (m_Grid.IsCellValueDisplayed(nRowIndex, nColumnIndex))
                {
        ...
        //``    (bool)cell.Value ? CheckBoxState.CheckedNormal : CheckBoxState.UncheckedNormal );
                    ...
                    else
                    {
                        ...
                        if (cell is DataGridViewTextBoxCell || cell is DataGridViewComboBoxCell)
                        {
                            ...
                            CellAlignmentToStringFormatAlignment(cellStyle.Alignment, strFormat);
        ...
        /// <summary>
        /// Gets the REAL cell style - the style after CellStyleSetting (called by CellFormatting) has been applied
        /// </summary>
        /// <param name="grid">The grid</param>
        /// <param name="rowIndex">Row index of cell</param>
        /// <param name="columnIndex">Column index of cell</param>
        /// <param name="cellStyle">Output parameter, cell style returned</param>
        public void GetCellStyle(DataGridView grid, int rowIndex, int columnIndex,
            out DataGridViewCellStyle cellStyle)
        {
            cellStyle = grid[columnIndex, rowIndex].InheritedStyle.Clone();

            // REMOVED SetCellStyle(rowIndex, columnIndex, this[columnIndex, rowIndex].Value, cellStyle, ref eColumnType);
        }
